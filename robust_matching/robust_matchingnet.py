...
class MatchingNet(nn.Module):
    ...
    def loss(self, sample):
        ...
        support_ = [item for xs_ in xs for item in xs_]
        query_ = [item for xq_ in xq for item in xq_]
        
        
        target_inds = torch.arange(0, n_class).view(n_class, 1, 1).expand(n_class, n_query, 1).long()
        #print("target_index",target_inds)
        target_inds = Variable(target_inds, requires_grad=False).to(device)
        
        target_inds_ = target_inds.reshape(-1)
        data = []
        for i in range(len(query_)):
            data.append((str(query_[i]),target_inds_[i].item()))
            
        
        
        text_ = textattack.datasets.Dataset(data) 
            #print("data: ",text_[1:3])
        ...
        attacker = textattack.Attacker(attack, text_, attack_args)

        attacker.attack_dataset()
        perturb_df = pd.read_csv('log.csv')
        perturb_data = []
        for i in range(perturb_df.shape[0]):
            perturb_data.append(perturb_df.iloc[i][1])
        
        
        x = support_ + perturb_data
...
if __name__ == "__main__":
    #load attacking model, set num_labels = number of classes in N-way K-shot learn setup
